...
class ImageColorCluster:
    def __init__(self, file_path, eps=10, min_samples=500):
        """
        파일 경로, eps, 및 min_samples로 ImageColorCluster 초기화
    ...
    def pixel_grid_images(self, cluster_image, sorted_cluster_labels, cluster_sizes_without_noise):
        ...
        for label in sorted_cluster_labels:
            ...
            print(f"Saved cluster grid image {label}: {saved_image.size}")
...
# 사용 예제
file_path = os.path.join("/home/jdk/project/function/check_image/00-0008_AC.png")
image_cluster = ImageColorCluster(file_path)
image_cluster.load_image()  # 이미지 로드 및 표시
image_cluster.extract_colors()  # 이미지에서 색상 추출
image_cluster.cluster_colors()  # 색상을 클러스터링하고 클러스터 표시
