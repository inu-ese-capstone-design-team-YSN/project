...
class AddaptiveConvolution:
    ...
    # 메인 함수 정의
    def AddaptiveConvolutionMode(self, mode, image_filename):
        if mode == 'TPG':
            image_path = f"{self.tpg_combined_image_dir}/{image_filename}_comb{self.extension}"
        elif mode == 'TCX':
        ...
        if mode == 'TPG':
            new_image_rgb.save(f"{}/{image_filename}{self.extension}")
        elif mode == 'TCX':
            new_image_rgb.save(f"{}/{image_filename}{self.extension}")
        elif mode == 'Swatch':
            new_image_rgb.save(f"{}/{image_filename}{self.extension}")
        ...
        print(f"{image_filename}_comb{self.extension} AC 보정 완료")

        

if __name__ == "__main__":
    ac = AddaptiveConvolution()
