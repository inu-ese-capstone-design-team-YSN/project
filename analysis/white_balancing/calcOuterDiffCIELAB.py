...
'''
2024.05.14 kwc
CIELAB2000색공간 기반 색온도 보정 알고리즘 개발
'''
...
for i in range(start_num, end_num + 1):
    ...
    # image를 np array로 변환
    np_image = np.array(original_image, dtype=np.float32) / 255.0
    
    # RGB to LAB 변환
    lab_image = color.rgb2lab(np_image)
    
    # 보정
    lab_image[..., 0] += (standard_lab[0] - total_lab_mean[0])
    lab_image[..., 1] += (standard_lab[1] - total_lab_mean[1])
    lab_image[..., 2] += (standard_lab[2] - total_lab_mean[2])
    
    # LAB to RGB 변환
    corrected_image = color.lab2rgb(lab_image)
    corrected_image = np.clip(corrected_image * 255, 0, 255).astype(np.uint8)

    # np array를 다시 image로 변환
    WB_image = Image.fromarray(corrected_image)
